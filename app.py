...
def init_db():
  with app.app_context():
    ...
    ''')
        ...
        # Correction ici: S'assurer que le nom de la table est bien 'cistern'
...
def get_fuel():
  ...
  if cistern_data:
    ...
    # Correction ici: 'cistern' au lieu de 'Citerne' pour la cohérence
...
def get_distribution_history():
  conn = get_db_connection()
  cursor = conn.cursor()
    # Correction ici: Noms de colonnes en anglais comme défini dans CREATE TABLE
...
def set_initial_fuel():
  ...
  try:
    ...
    conn = get_db_connection()
    cursor = conn.cursor()
        # Le nom de la table était déjà correct ici ('cistern')
